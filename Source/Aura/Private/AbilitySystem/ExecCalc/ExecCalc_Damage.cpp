	...
	for(TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if(TypeDamage > -.5f)  // .5 padding for floating point precision
		{
			...
			if(bDebuff)
			{
				...
				const float DebuffFrequence = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Frequency, false, -1.f);

				UAuraAbilitySystemLibrary::SetDebuffDamage(ContextHandle, DebuffDamage);
				UAuraAbilitySystemLibrary::SetDebuffDuration(ContextHandle, DebuffDuration);
				UAuraAbilitySystemLibrary::SetDebuffFrequency(ContextHandle, DebuffFrequence);
			}
		}
	}
}
...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistance)
	{
		...
		checkf(TagsToCaptureDefs.Contains(ResistanceTag), TEXT("TagsToCaptureDef doesn't contain Tag: [%s] in ExecCala_Damage"), *ResistanceTag.ToString());
		...
		if(UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			UGameplayStatics::ApplyRadialDamageWithFalloff(
				...
				1.f, UDamageType::StaticClass(), TArray<AActor*>(), SourceAvatar,
				nullptr)
			;
		}
