...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GourndLocation, int32 NumPoints, float YawOverride)
{
	checkf(ImmutablePts.Num() >= NumPoints, TEXT("Attempted to access ImmutalbePts out of bounds."));
	...
	for(USceneComponent* Pt : ImmutablePts)
	{
		...
		const FVector LowerdLocation = FVector(Pt->GetComponentLocation().X, Pt->GetComponentLocation().Y, Pt->GetComponentLocation().Z-500.f);
		...
		GetWorld()->LineTraceSingleByProfile(HitResult, RaisedLocation, LowerdLocation, FName("BlockAll"), QueryParams);
